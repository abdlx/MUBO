...
export default function BrowseClient({ kind, initialQuery = "" }: { kind: string; initialQuery?: string }) {
  const { tracks, currentTrack, isPlaying, isLoading, playQueue, togglePlay, openPlayer } = usePlayer();
  ...
  return <main className={styles.shell}>
    <div className={styles.content}>
      <header className={styles.header}><Link href="/" aria-label="Back to home" className={styles.back}>‹</Link><div><p className={styles.eyebrow}>Mubo · Explore</p><h1>{title}</h1><span>{selectedPlaylist || selectedGenre ? `${selected.length} songs` : "Everything in your music library"}</span></div></header>
      <label className={styles.search}><span aria-hidden="true">⌕</span><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search songs, artists, albums, and genres" autoFocus={activeKind === "all"} aria-label="Search your library" />{query && <button onClick={() => setQuery("")} aria-label="Clear search">×</button>}</label>
      <nav className={styles.tabs} aria-label="Browse categories">{kinds.map(item => <Link key={item} className={item === activeKind ? styles.activeTab : ""} href={`/browse/${item}`}>{item[0].toUpperCase() + item.slice(1)}</Link>)}</nav>
      ...
      {(activeKind === "all" || activeKind === "genres") && !selectedGenre && !selectedPlaylist && genres.length > 0 && <section className={styles.section}><div className={styles.sectionHead}><h2>Genres</h2>{activeKind === "all" && <Link href="/browse/genres">View all →</Link>}</div><div className={styles.genreGrid}>{genres.filter(name => !normalized || name.toLowerCase().includes(normalized)).map((name, index) => <button className={styles.genre} style={{ "--hue": `${(index * 47 + 300) % 360}` } as React.CSSProperties} key={name} onClick={() => setSelectedGenre(name)}><strong>{name}</strong><span>{tracks.filter(track => track.genres.includes(name)).length} songs</span></button>)}</div></section>}
      {(activeKind === "all" || activeKind === "playlists") && !selectedGenre && !selectedPlaylist && <section className={styles.section}><div className={styles.sectionHead}><h2>Playlists</h2>{activeKind === "all" && <Link href="/browse/playlists">View all →</Link>}</div><div className={styles.genreGrid}>{playlists.filter(item => !normalized || item.name.toLowerCase().includes(normalized)).map((item, index) => <button className={styles.genre} style={{ "--hue": `${(index * 53 + 210) % 360}` } as React.CSSProperties} key={item.name} onClick={() => setSelectedPlaylist(item.name)}><strong>{item.name}</strong><span>{item.items.length} songs</span></button>)}</div></section>}
