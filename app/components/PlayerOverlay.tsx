...
export default function PlayerOverlay() {
  const { currentTrack, currentTime, duration, isPlaying, playerOpen, closePlayer, togglePlay, nextTrack, prevTrack, seek, queue, shuffleMode, setShuffleMode, repeatMode, cycleRepeat, playQueue } = usePlayer();
    ...
    {currentTrack && <><div className={styles.backdrop}>{currentTrack.coverImage && <Image src={currentTrack.coverImage} alt="" fill sizes="100vw" />}</div><div className={styles.wash} />
      <div className={styles.page} role="dialog" aria-modal="true" aria-label="Now playing">
        ...
        <section className={styles.controls}>
          ...
          <input className={styles.progress} aria-label="Song progress" type="range" min="0" max={total || 1} value={Math.min(currentTime, total || 1)} onChange={event => seek(Number(event.target.value))} disabled={!total} style={{ "--fill": `${total ? currentTime / total * 100 : 0}%` } as React.CSSProperties} />
          <div className={styles.times}><span>{format(currentTime)}</span><span>{total ? format(total) : "--:--"}</span></div>
