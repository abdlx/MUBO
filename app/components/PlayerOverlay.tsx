    ...
    {currentTrack && <><div className={styles.backdrop} /><div className={styles.wash} />
      <div className={styles.page} role="dialog" aria-modal="true" aria-label="Now playing">
        ...
        <section className={styles.controls}>
          <div className={styles.trackHeading}><div><h1>{currentTrack.title}</h1><Link href={currentTrack.artistIds[0] ? `/artist/${currentTrack.artistIds[0]}` : "/browse/artists"}>{currentTrack.artist}</Link></div><button onClick={toggleLike} aria-label={likedTrackIds.includes(currentTrack.id) ? "Unlike track" : "Like track"} title={likedTrackIds.includes(currentTrack.id) ? "Liked" : "Like"}>{likedTrackIds.includes(currentTrack.id) ? "♥" : "♡"}</button></div>
          ...
          <div className={styles.queueHead}><div><strong>Up next</strong><span>{queue.length} songs · {shuffleMode === "smart" ? "Genre smart shuffle" : shuffleMode === "standard" ? "Standard shuffle" : "In order"}</span></div><button onClick={() => startRadio("track", currentTrack)} aria-label="Start track radio">Track radio ↗</button></div>
          <div className={styles.queue}>{queue.filter(item => item.id !== currentTrack.id).slice(0, 5).map(item => <button key={item.id} onClick={() => playTrack(item)}><span className={styles.queueArt}>{item.coverImage && <Image src={item.coverImage} alt="" fill sizes="36px" />}</span><span><strong>{item.title}</strong><small>{item.artist}{recommendedTrackIds.includes(item.id) ? " · ✦ Smart Shuffle" : ""}</small></span><span>▶</span></button>)}</div>
        </section>
      </div></>}
  </div>;
}
